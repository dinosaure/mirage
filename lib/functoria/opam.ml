...
let v ?(build = []) ?(install = Install.empty) ?(depends = []) ?(pins = []) ~target ~src
    name =
  let src =
...
depends: [%a]
%a%a|}
        t.name t.name pp_build t.build Install.pp_opam t.install pp_packages t.depends
        pp_src t.src pp_pins t.pins
  | `Local ->
      Fmt.pf ppf
        {|opam-version: "2.0"
name: "%s"
...
bug-reports: "dummy"
dev-repo: "git://dummy"
synopsis: "Unikernel %s - local dependencies"

depends: [%a]
|}
        t.name t.name pp_packages t.depends
