 ...
 *)
 ...
 module Make (P : S) = struct
   ...
   let dune_workspace_path t = Fpath.(build_dir t / P.name /  "dune-workspace.config")
   ...
   let build_config_exe t ?ppf ?err_ppf () =
     let dune_workspace_path = dune_workspace_path t in
     let command =
       Bos.Cmd.(v "dune" % "build" % p Fpath.( build_dir t / "config.exe" ) % "--root" % "." % "--workspace" % p dune_workspace_path)
     in
   ...
   let clean args ?ppf ?err_ppf argv =
     ...
     let* () =
       ...
       if is_file then
         try_to_re_exec args ?ppf ?err_ppf argv
       else Action.ok ()
     in
     clean_files args
