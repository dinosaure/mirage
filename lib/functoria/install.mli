...
(** [v ~bin:[(src,dst),...] ~etc ()] is the installation of [src] as [dst] as binary files, and [etc] as configuration. *)
...
val dune : 
  ...
  context_name:string ->
  t -> Dune.t
(** [dune ~build_dir ~context_name ()] is the dune rules to promote installed files back in the source tree. *)
