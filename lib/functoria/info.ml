...
type t = {
  ...
  opam_local : install:Install.t -> Opam.t;
  opam_global : install:Install.t -> Opam.t;
}
...
let opam kind t =
  match kind with `Local -> t.opam_local | `Global -> t.opam_global
...
let v ~packages ~keys ~context ~build_cmd ~src name =
  ...
  let local_packages, global_packages =
    List.partition (fun pkg -> Package.kind pkg == `Local) packages
  in
  let opam_local ~install:_ =
    Opam.v ~depends:local_packages ~pins:(pins local_packages) ~target:`Local
      ~build:build_cmd ~src name
  in
  let opam_global ~install =
    Opam.v ~depends:global_packages ~install ~pins:(pins global_packages)
      ~build:build_cmd ~target:`Global ~src name
  in
  let packages =
    List.fold_left
      (fun m p ->
  ...
  in
  { name; keys; packages; context; output = None; opam_local; opam_global }
