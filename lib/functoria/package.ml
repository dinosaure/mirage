...
let merge a b =
  if a.name <> b.name then None
  else if a.kind <> b.kind then None
  else
    let name = a.name in
    ...
    and kind = a.kind in
    match pin with
    | None -> Some { name; build; kind; libs; min; max; pin }
    | Some _ ->
        (* pin wins over min and max *)
        let empty = String.Set.empty in
        Some { name; build; kind; libs; min = empty; max = empty; pin }

let v ?(kind = `Local) ?(build = false) ?sublibs ?libs ?min ?max ?pin name =
  let libs =
  ...
  { name; build; kind; libs; min; max; pin }
