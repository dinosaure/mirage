...
type target = [ solo5_target | xen_target ]

let cast = function #target as t -> t | _ -> invalid_arg "not a solo5 target."
...
(rule
 ...
 (action (with-stdout-to %%{target}
  ...
  |}
    (flags_file target) Key.pp_target target
...
let dune i = [ main i; manifest i; ldflags i; link i; alias_override i ]
...
let install i =
  ...
  let additional_artifacts =
    match target with `Xen -> [v (name ^ ".xl"); v (name ^ ".xl.in") ] | _ -> []
  in 
